...
while getopts ':hs:z:c:e:a:p:r:v:' option; do
  case "$option" in
  h)
    echo "$usage"
    exit 1
...
run_phenix() {
  mkdir_and_cd ../phenix/
  for file in ../Rosetta/poly_"${res}"/*.pdb; do
